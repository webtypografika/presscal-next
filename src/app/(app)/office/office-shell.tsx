  ...
  return (
    <div style={{ display: 'flex', gap: 0, height: 'calc(100vh - 140px)', marginTop: -8 }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {/* Items — card layout grouped by first tag */}
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px 16px' }}>
          ...
          {(() => {
            ...
            for (const item of items) {
              const cat = item.tags[0] || '_uncategorized';
              if (!groups[cat]) groups[cat] = [];
              groups[cat].push(item);
            }
            const sortedKeys = Object.keys(groups).filter(k => k !== '_uncategorized');
            if (groups['_uncategorized']) sortedKeys.push('_uncategorized');
            return sortedKeys.map(cat => (
              <div key={cat} style={{ marginBottom: 16 }}>
                <div style={{
                  fontSize: '0.65rem', fontWeight: 800, color: cat === '_uncategorized' ? '#475569' : activeProject?.color || 'var(--text-muted)',
                  letterSpacing: '0.05em', padding: '4px 0 6px',
                  borderBottom: '1px solid var(--border)', marginBottom: 8,
                  textTransform: 'uppercase',
                }}>
                  {cat === '_uncategorized' ? 'Χωρίς κατηγορία' : `#${cat}`}
                  <span style={{ fontWeight: 500, marginLeft: 6, color: '#475569' }}>({groups[cat].length})</span>
                </div>
